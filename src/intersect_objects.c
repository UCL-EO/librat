     ...
     int 			*tree_depth_Ptr;
{
  ...
  */
  ...
  if(hitMe && flagbag->sunlocation && flagbag->isSunTest){
    ...
    */
    if(flagbag->lengthToSun <= objectlist_Ptr->RayLength)
      hitMe=0;
  }
...
double lidarDistance(materialbag,ipray,illumination,flagbag)
     MaterialBag		*materialbag;
     Ray			*ipray;
     IlluminationBag		*illumination;
     FlagBag			*flagbag;
{
  ...
  if(flagbag->sunlocation){
    ...
    flagbag->illuminationvector=normalise(V2);
    d=V_dot(flagbag->illuminationvector,*illumination->sun_Ptr);
    flagbag->lengthToSun=distanceBackToViewer=V_mod(V2);
    flagbag->sunrayangle=acos(d)*180./M_PI;
  }else return(0.);
     ...
     int                        *tree_depth_Ptr,hitCamera;
{
  ...
  if(! hitCamera ){
    ...
    for(l=0;l<bb->nSuns;l++)hitPoint->distanceBackToViewer[l]=lidarDistance(materialbag,ipray,&illumination[l],flagbag); 
  ...
  if(material_Ptr){
    ...
    */
    ...
    lidarDistance(materialbag,ipray,illumination,flagbag);
    /* hit sky */
    hitPoint->hitSky=1;
    return(diffuseRay);
  }
