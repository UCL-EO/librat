  ...
  */
  ...
  if(ratObj->flagbag->sunlocation || illuminationPosition){
    if(!ratObj->flagbag->sunlocation){
      ratObj->flagbag->sunlocation=(double *)v_allocate(3,sizeof(double));
    }
    if(illuminationPosition){
      for(i=0;i<3;i++)ratObj->flagbag->sunlocation[i]=illuminationPosition[i]; 
    }
/* this means its been set on the cmd line */
    if(!ratObj->illumination->position){
      ...
      ratObj->illumination->position=(Camera_op *)v_allocate(nSuns,sizeof(Camera_op));
      for(i=0;i<nSuns;i++){
        ratObj->illumination[i].position->ppoint.x=ratObj->flagbag->sunlocation[0];
        ratObj->illumination[i].position->ppoint.y=ratObj->flagbag->sunlocation[1];
        ratObj->illumination[i].position->ppoint.z=ratObj->flagbag->sunlocation[2];
      }
    }
  }
...
/* only do these if things have changed */
  ...
  */
  ...
  for(rtd=0;rtd<=nRTD;rtd++){ 
    ...
    if(ratObj->flagbag->sunlocation){
      ratObj->ratTree->lengthToSun[rtd]=ratObj->flagbag->lengthToSun;
      ratObj->ratTree->angleToSun[rtd]=ratObj->flagbag->sunrayangle;
    } 
